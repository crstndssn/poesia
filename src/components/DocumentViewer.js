...
const DocumentViewer = ({ fileUrl, isOpen, onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center overflow-hidden ${
        isOpen ? 'block' : 'hidden'
      }`}
    >
      <div className="bg-white rounded-lg p-6 w-full md:w-full shadow-lg relative">
        <button
